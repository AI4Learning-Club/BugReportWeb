...
若希望通过 ChatGPT、Claude、Cursor 等 AI **代用户调用本 API**（登记 Bug、登记功能、查询列表等），请使用 **[提示词参考](/docs/prompts)** 中的可复制提示词。

要点摘要：

| 要求 | 说明 |
|------|------|
| 以文档为准 | OpenAPI + 本指南 + AI 指南，不编造端点 |
| 先索要凭证 | API 基址、`username`、`password` → `POST /auth/login` |
| 再预检权限 | `GET /auth/me`，确认 `ACTIVE` 与目标 Permission |
| 登记 Bug | `GET /systems` → `POST /bugs`（`CREATE_BUG`） |
| 登记功能 | `GET /systems` → `POST /features`（`CREATE_FEATURE`） |

终端用户可使用提示词参考 §6 的简短开场白，无需了解 REST 细节。
...
- **[AI 代理指南](/docs/ai-guide)**：决策树、禁止事项与检查清单
- **[提示词参考](/docs/prompts)**：可复制系统提示词与任务提示词（含鉴权引导）
